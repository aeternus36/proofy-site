...
// Target size (may be adjusted to integer module sizes)
var targetW=this._htOption.width,
targetH=this._htOption.height,
target=Math.min(targetW,targetH);

// QR modules
var mCount=this._oQRCode.getModuleCount();

// Quiet zone (margin) in modules
var margin=0;
if (typeof this._htOption.margin === "number") margin=this._htOption.margin|0;
if (margin<0) margin=0;

// Total module grid including quiet zone
var total=mCount + margin*2;

// Choose integer module size (critical for scannability)
var moduleSize=Math.floor(target / total);
if (moduleSize<1) moduleSize=1;

// Final canvas size (exact integer multiple)
...
// Solid background
e.fillStyle=this._htOption.colorLight;
e.fillRect(0,0,size,size);

// Draw modules
e.fillStyle=this._htOption.colorDark;
for (var row=0; row<mCount; row++){
  for (var col=0; col<mCount; col++){
    if (this._oQRCode.isDark(row,col)){
      ...
      e.fillRect(x,y,moduleSize,moduleSize);
    }
  }
}
return t
...
window.QRCode=c;window.QRCodeCorrectLevel=l;})();
