...
/*
  IMPORTANT PATCH:
  Render with integer cellSize + real quiet-zone margin in the canvas.
  This removes moiré/diagonal artifacts and makes the QR reliably scannable.
*/
c.prototype={
  makeCode:function(t){
  ...
  _createCanvas:function(){
    ...
    var mc=this._oQRCode.getModuleCount();
    var margin=(this._htOption.margin!=null?this._htOption.margin:4);
    if(margin<0) margin=0;

    // Choose an integer cellSize so modules align perfectly.
    var target=Math.min(this._htOption.width,this._htOption.height);
    var cellSize=Math.floor(target/(mc+margin*2));
    if(cellSize<1) cellSize=1;

    var size=(mc+margin*2)*cellSize;
    canvas.width=size;
    canvas.height=size;
