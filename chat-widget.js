(() => {
  function init() {
    ...
    style.textContent = `
      ...
      .proofy-panel{position:fixed;bottom:86px;right:20px;z-index:9999;width:min(380px, calc(100vw - 40px));height:min(560px, calc(100vh - 140px));background:rgba(10,16,32,.92);border:1px solid rgba(255,255,255,.10);border-radius:18px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.45);backdrop-filter: blur(10px);display:none;font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial;color:#eaf1ff;}
      ...
      try {
        ...
        const answer =
          typeof data?.answer === "string" && data.answer.trim()
            ...
            ? data.reply.trim()
            : "Jag kunde inte läsa svaret just nu. Mejla kontakt@proofy.se så hjälper vi dig.";
    ...
    addMessage("assistant", "Hej. Vad vill du verifiera, och i vilket sammanhang (revision, bokslut, tvist, spårbarhet)?");
  }
