export async function onRequestPost(context) {
  try {
    ...
    const volume = (form.get("volume") || "").toString().trim();
    ...
    // Enkla format-checks (snälla men tydliga)
    ...
    // --- KONFIG ---
    // Sätt dessa i Cloudflare Pages: Settings → Environment variables
    // CONTACT_TO = "kontakt@proofy.se"
    // CONTACT_FROM = "no-reply@proofy.se"  (bör vara en adress på din domän)
    ...
    const subject = `Demo/Pilot-förfrågan${subjectCompany}`;

    const lines = [
      "Ny förfrågan från proofy.se",
      "",
      `Namn: ${name}`,
      `E-post: ${email}`,
    ...
    // MailChannels (fungerar bra på Cloudflare)
    const payload = {
      personalizations: [
        { to: [{ email: TO }] }
      ],
    ...
    if (!r.ok) {
      ...
      // Logga för felsökning i Cloudflare
    ...
    // Redirect till tack-sida (du har redan /thanks.html)
    return Response.redirect(`${new URL(req.url).origin}/thanks.html`, 302);

  } catch (e) {
    console.log("Contact form exception:", e);
